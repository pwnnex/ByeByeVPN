...
The v2.5.9 uTLS dual-probe adds two extra TLS handshakes per TLS
...
Windows: download `byebyevpn-v2.5.9-win64.zip` from
...
| 3b | TCP stack fingerprint (v2.5.9)  | Распределение handshake-времени + SIO_TCP_INFO peer window/MSS + поведение на закрытом порту, без админа |
...
| 5b | uTLS dual-probe + JA4 (v2.5.9)  | По два TLS handshake на TLS-порт (chrome-flavored ctx vs openssl-default), JA4 / JA4S из захваченных байт CH/SH |
...
Windows: скачать `byebyevpn-v2.5.9-win64.zip` со страницы
